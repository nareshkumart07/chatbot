...
def load_and_cache_model(model_key, loader_func, *args):
    ...
    if model_key not in st.session_state:
        with st.spinner(f"Loading {model_key.replace('_', ' ')}... This may take a moment."):
...
def ask_query(query, model_choice):
    """
    ...
    try:
        if 'BART' in model_choice:
            ...
            return f"**Answer generated by BART:**\n\n{summary}", doc_context

        elif 'Flan-T5' in model_choice:
            t5_tokenizer = load_and_cache_model('t5_tokenizer', T5Tokenizer.from_pretrained, "google/flan-t5-large")
            t5_model = load_and_cache_model('t5_model', T5ForConditionalGeneration.from_pretrained, "google/flan-t5-large")
            input_text = f"question: {query} context: {doc_context}"
            inputs = t5_tokenizer(input_text, return_tensors="pt", max_length=512, truncation=True)
            outputs = t5_model.generate(**inputs, max_length=150, min_length=30, num_beams=4, early_stopping=True)
            answer = t5_tokenizer.decode(outputs[0], skip_special_tokens=True)
            return f"**Answer generated by Flan-T5:**\n\n{answer}", doc_context
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Choose a model:",
        ('Advanced Model (BART)', 'Fast Model (Flan-T5)'),
        help="BART is great for summarization. Flan-T5 is excellent for direct question answering."
    )
    st.warning("Models can take a few minutes to download the first time you select them.", icon="⏳")
    st.markdown("---")
...
if user_query := st.chat_input("Ask a question about your document..."):
    if not uploaded_file:
        ...
        with st.chat_message("assistant"):
            spinner_message = "Finding answer with " + model_choice.split('(')[1].replace(')', '')
            with st.spinner(spinner_message):
                response, context = ask_query(user_query, model_choice)
                if response:
